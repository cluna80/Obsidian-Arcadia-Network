 ...
 */
export function findInteropLogIndex(receipt, sender) {
    ...
    const l2ToL1LogIndex = receipt.l2ToL1Logs.findIndex((log) => {
        const senderMatches = log.sender.toLowerCase() === utils.L1_MESSENGER_ADDRESS.toLowerCase();
 ...
 */
export async function getGatewayProof(l2, txHash, logIndex) {
 ...
 */
export async function getGwBlockForBatch(batch, l2, gw, pollMs = 1000) {
    for (;;) {
        const details = await l2.getL1BatchDetails(Number(batch));
        const exec = details?.executeTxHash && details.executeTxHash !== ethers.ZeroHash
 ...
 */
export async function waitForGatewayInteropRoot(gwChainId, target, gwBlock, { timeoutMs = 120000, pollMs = 1500, } = {}) {
    const interop = new ethers.Contract(utils.L2_INTEROP_ROOT_STORAGE_ADDRESS, utils.L2_INTEROP_ROOT_STORAGE_ABI, target);
...
export function classifyPhase(d) {
    if (d.status === 'failed')
        return 'FAILED';
