...
import { classifyPhase, findInteropLogIndex, getGatewayProof, getGwBlockForBatch, waitForGatewayInteropRoot, } from './interop-utils';
import { isAddressEq, L2_MESSAGE_VERIFICATION_ABI, L2_MESSAGE_VERIFICATION_ADDRESS, } from './utils';
 ...
 * ```
 */
export function resolveGateway(config = {}) {
...
export class InteropClient {
    ...
    async sendMessage(srcWallet, message) {
        const messenger = new ethers.Contract(utils.L1_MESSENGER_ADDRESS, utils.L1_MESSENGER, srcWallet);
    ...
    async verifyMessage(params) {
        ...
        const verifier = new ethers.Contract(L2_MESSAGE_VERIFICATION_ADDRESS, L2_MESSAGE_VERIFICATION_ABI, targetChain);
        ...
        const result = {
            source: {
                ...
                messageHash: ethers.keccak256(msgData.data),
            },
    ...
    async getVerificationArgs(params) {
        ...
        const { l2ToL1LogIndex, messageSentInContract } = findInteropLogIndex(finalizedRcpt, sender);
        if (l2ToL1LogIndex < 0) {
            throw new Error('Interop log not found in source receipt for L1Messenger');
        }
        const log = finalizedRcpt.logs.filter(log => isAddressEq(log.address, utils.L1_MESSENGER_ADDRESS) &&
            log.topics[0] === ethers.id('L1MessageSent(address,bytes32,bytes)'))[l2ToL1LogIndex];
        const messageHex = ethers.AbiCoder.defaultAbiCoder().decode(['bytes'], log.data)[0];
        if (!messageHex) {
        ...
        const { nodes, proofId } = await getGatewayProof(srcProvider, txHash, l2ToL1LogIndex);
        const gwBlock = await getGwBlockForBatch(BigInt(l1BatchNumber), srcProvider, this.gwProvider);
        const interopRoot = await waitForGatewayInteropRoot(this.gwChainId, targetChain, gwBlock, { timeoutMs });
