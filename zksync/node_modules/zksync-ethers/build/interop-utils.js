...
function findInteropLogIndex(receipt, sender) {
    const paddedSender = ethers_1.ethers.zeroPadValue(sender, 32);
    const paddedContract = receipt.to
        ? ethers_1.ethers.zeroPadValue(receipt.to, 32)
        : null;
...
async function getGwBlockForBatch(batch, l2, gw, pollMs = 1000) {
    for (;;) {
        ...
        const exec = details?.executeTxHash && details.executeTxHash !== ethers_1.ethers.ZeroHash
